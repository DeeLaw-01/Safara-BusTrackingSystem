  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="relative bg-gradient-to-br from-stone-900 via-stone-800 to-stone-900 overflow-hidden">
        {/* Ambient glow */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-amber-500/[0.07] rounded-full blur-[120px]" />
        <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-orange-500/[0.05] rounded-full blur-[80px]" />
          ...
          {!assignedBus ? (
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-6 sm:gap-10">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-gradient-to-br from-amber-400/20 to-amber-600/20 rounded-2xl flex items-center justify-center border border-amber-500/20">
                  <Bus className="w-8 h-8 text-amber-400" />
                </div>
                <div>
                  <p className="text-xs text-stone-500 uppercase tracking-wider">Your Bus</p>
                  <h2 className="text-2xl font-bold text-white">{assignedBus.name}</h2>
                  <p className="text-sm text-stone-400">{assignedBus.plateNumber}</p>
                </div>
              </div>
              ...
              <div className="flex gap-6 sm:gap-8 text-sm pb-1">
                <div>
                  <p className="text-stone-500 text-xs">Capacity</p>
                  <p className="text-white font-semibold text-lg">{assignedBus.capacity} <span className="text-stone-500 text-xs font-normal">seats</span></p>
                </div>
                {routeName && (
                  <div>
                    <p className="text-stone-500 text-xs">Route</p>
                    <p className="text-amber-400 font-semibold text-lg flex items-center gap-1.5">
                ...
                <div>
                  ...
                  {currentTrip ? (
                    <p className="text-orange-400 font-semibold flex items-center gap-1.5 text-lg">
                      <span className="w-2 h-2 bg-orange-400 rounded-full pulse-live" />
                      Live
                    </p>
                  ) : (
                    <p className="text-stone-400 font-semibold text-lg">Idle</p>
                  )}
                </div>
              </div>
            </div>
        ...
        {assignedBus && (
          <Link
            ...
            className={`group block rounded-2xl p-6 shadow-lg border transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 ${
              currentTrip
                ? 'bg-gradient-to-r from-orange-500 to-orange-600 border-orange-400/50 text-white'
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${
                  currentTrip ? 'bg-white/15' : 'bg-amber-50'
                }`}>
                  <Navigation className={`w-7 h-7 ${currentTrip ? 'text-white' : 'text-amber-600'}`} />
                </div>
                <div>
                  {currentTrip ? (
                    <>
                    ...
                    <>
                      <h3 className="text-xl font-bold text-stone-900">Start a New Trip</h3>
                      <p className="text-sm text-stone-500 mt-0.5">Begin broadcasting your location to riders on your route</p>
                    </>
                  )}
                </div>
              </div>
              ...
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center group-hover:translate-x-1 transition-transform ${
                currentTrip ? 'bg-white/10' : 'bg-stone-100 group-hover:bg-amber-50'
              }`}>
                <ChevronRight className={`w-5 h-5 ${currentTrip ? 'text-white' : 'text-stone-400 group-hover:text-amber-600'}`} />
              </div>
            </div>
          </Link>
        )}
        ...
        {assignedBus && (
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border border-stone-200 p-4 text-center">
              ...
              <div className="text-2xl font-bold text-stone-900">{recentTrips.length}</div>
              <div className="text-xs text-stone-400 mt-0.5">Total Trips</div>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 p-4 text-center">
              <div className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center mx-auto mb-2">
                <UsersIcon className="w-4 h-4 text-blue-600" />
              </div>
              <div className="text-2xl font-bold text-stone-900">{assignedBus.capacity}</div>
              <div className="text-xs text-stone-400 mt-0.5">Bus Capacity</div>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 p-4 text-center">
              <div className="w-8 h-8 bg-amber-50 rounded-lg flex items-center justify-center mx-auto mb-2">
              ...
              <div className="text-2xl font-bold text-stone-900">
                {currentTrip ? <span className="text-orange-500">ON</span> : <span className="text-stone-300">OFF</span>}
              </div>
              <div className="text-xs text-stone-400 mt-0.5">Live Status</div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm">
          <div className="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-stone-800">Trip History</h3>
            </div>
            <span className="text-xs text-stone-400">{recentTrips.length} trips</span>
          </div>
          ...
          {recentTrips.length === 0 ? (
            <div className="text-center py-16 px-6">
              ...
              <p className="text-stone-500 font-medium">No trips yet</p>
                      ...
                      <div key={trip._id} className="relative flex gap-4 py-3.5 group">
                        ...
                        <div className="relative z-10 mt-1">
                          <div className={`w-[22px] h-[22px] rounded-full border-[3px] ${
                            isOngoing
                              ? 'bg-orange-500 border-orange-200 pulse-live'
                        ...
                        <div className="flex-1 flex items-center justify-between min-w-0 -mt-0.5">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium text-stone-700 truncate">{routeLabel}</span>
                              {isOngoing && (
                                <span className="text-[10px] bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded font-semibold uppercase tracking-wider">Live</span>
                              )}
                            </div>
