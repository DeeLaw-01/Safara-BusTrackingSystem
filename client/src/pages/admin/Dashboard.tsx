...
const busIcon = L.divIcon({
  ...
  html: `<div style="
    width: 32px; height: 32px; background: linear-gradient(135deg, #A0714E, #8B5E3C);
...
export default function AdminDashboard() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-stone-400">Loading dashboard...</span>
        </div>
      </div>
    );
  }
  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        <p className="text-stone-500">Failed to load dashboard data.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 stagger-children">
      ...
      <div className="relative bg-gradient-to-r from-amber-600 via-amber-500 to-orange-500 animated-gradient rounded-2xl p-6 lg:p-8 text-white overflow-hidden scan-line-overlay">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-4 mt-4">
            <div className="flex items-center gap-1.5 text-xs text-white/70">
              <div className="w-2 h-2 bg-orange-300 rounded-full pulse-live" />
              System Online
            </div>
            <div className="flex items-center gap-1.5 text-xs text-white/70">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          accent="teal"
          chart={<MiniBarChart values={[riderPct, driverPct, adminPct]} colors={['#A0714E', '#C67D4A', '#6366f1']} />}
        />
        <StatCard
          icon={UserCheck}
          ...
          accent="amber"
          link="/admin/users"
          chart={<MiniDonut value={stats.users.pendingDrivers} max={stats.users.drivers || 1} color="#C67D4A" />}
        />
        <StatCard
          icon={Bus}
          label="Live Trips"
          ...
          accent="emerald"
          chart={<MiniBarChart values={[stats.trips.ongoing * 20, stats.trips.today * 10, 60, 40]} colors={['#B8860B', '#B8860B', '#d1fae5', '#d1fae5']} />}
        />
        <StatCard
          icon={RouteIcon}
          label="Active Routes"
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-stone-800">Live Bus Map</h3>
                <p className="text-xs text-stone-400">{liveLocations.length} active now</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5 text-xs text-orange-600 bg-orange-50 px-2.5 py-1 rounded-full">
              <div className="w-1.5 h-1.5 bg-orange-500 rounded-full pulse-live" />
              Live
            </div>
          </div>
                ...
                <Marker key={bus.busId} position={[bus.latitude, bus.longitude]} icon={busIcon}>
                  <Popup>
                    <div className="p-1 min-w-[100px]">
                      <div className="text-xs font-medium text-stone-500 uppercase tracking-wider mb-1">Live Bus</div>
                      <div className="text-sm font-bold text-stone-800 mb-1 flex items-center gap-1.5">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-stone-800">Recent Trips</h3>
              </div>
              <span className="text-xs text-stone-400">{stats.trips.today} today</span>
            </div>
            <div className="p-3 max-h-[240px] overflow-y-auto">
              {recentTrips.length === 0 ? (
                <div className="text-center py-8">
                  <Clock className="w-8 h-8 text-stone-200 mx-auto mb-2" />
                  <p className="text-sm text-stone-400">No recent trips</p>
                </div>
              ) : (
                <div className="space-y-1.5">
                  {recentTrips.map((trip) => (
                    <div key={trip._id} className="flex items-center justify-between p-2.5 rounded-xl hover:bg-stone-50 transition-colors">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${
                          trip.status === 'ongoing' ? 'bg-orange-500 pulse-live' : 'bg-stone-300'
                        }`} />
                      ...
                      <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${
                        trip.status === 'ongoing' ? 'bg-orange-50 text-orange-700' : 'bg-stone-100 text-stone-500'
                      }`}>
                        {trip.status}
                      </span>
                    </div>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-5">
            <h3 className="text-sm font-semibold text-stone-800 mb-4">User Breakdown</h3>
            <div className="flex items-center gap-6">
              <div className="relative w-20 h-20 shrink-0">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                  <circle cx="50" cy="50" r="38" fill="none" stroke="#f1f5f9" strokeWidth="10" />
                  <circle cx="50" cy="50" r="38" fill="none" stroke="#A0714E" strokeWidth="10"
                  ...
                  />
                  <circle cx="50" cy="50" r="38" fill="none" stroke="#C67D4A" strokeWidth="10"
                    strokeDasharray={`${driverPct * 2.39} 239`}
                    strokeDashoffset={`-${riderPct * 2.39}`}
                  />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-base font-bold text-stone-800">{totalUsers}</span>
                </div>
              </div>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded bg-amber-500" />
                  <span className="text-stone-600">Riders ({stats.users.riders})</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded bg-amber-500" />
                  <span className="text-stone-600">Drivers ({stats.users.drivers})</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded bg-indigo-500" />
                  <span className="text-stone-600">Admins ({totalUsers - stats.users.riders - stats.users.drivers})</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          ...
          const colorMap = {
            teal: 'bg-amber-50 text-amber-600 group-hover:bg-amber-100',
            blue: 'bg-blue-50 text-blue-600 group-hover:bg-blue-100',
            amber: 'bg-amber-50 text-amber-600 group-hover:bg-amber-100',
          };
          return (
            <Link key={to} to={to} className="group bg-white rounded-2xl border border-stone-200 p-5 hover:border-amber-200 hover:shadow-md transition-all duration-200">
              ...
              <h3 className="text-sm font-semibold text-stone-800 mb-0.5">{label}</h3>
...
function StatCard({ icon: Icon, label, value, subValue, accent, link, chart }: StatCardProps) {
  const accentMap = {
    teal:    { bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-l-amber-500' },
    amber:   { bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-l-amber-500' },
    emerald: { bg: 'bg-orange-50', text: 'text-orange-600', border: 'border-l-orange-500' },
    blue:    { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-l-blue-500' },
  };
  const a = accentMap[accent];

  const content = (
    <div className={`bg-white rounded-2xl border border-stone-200 border-l-4 ${a.border} p-5 shadow-sm ${
      link ? 'hover:shadow-md hover:border-stone-300 transition-all cursor-pointer' : ''
    }`}>
      <div className="flex items-start justify-between mb-3">
      ...
      <div className="text-3xl font-bold text-stone-900 shimmer-text mb-0.5">{value}</div>
